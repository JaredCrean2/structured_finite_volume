...
TEST_F(DiscIfaceTester, RegularInterface)
{
  ...
  const mesh::AdjacentBlockIndexer& indexerL = iface.getAdjacentBlockIndexerL();
  ...
  i_corner = *iface.getOwnedBoundaryCellsR().getXRange().begin();
  j_corner = *iface.getOwnedBoundaryCellsR().getYRange().begin();
  const mesh::AdjacentBlockIndexer& indexerR = iface.getAdjacentBlockIndexerR();
